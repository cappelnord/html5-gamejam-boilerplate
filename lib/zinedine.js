...
Zinedine.hasAPI = function() {
	return (typeof(webkitAudioContext) == "function") || (typeof(webkitAudioContext) == "object");
};
...
Zinedine.init = function() {
	if(Zinedine.hasAPI()) {
		Zinedine.ctx = new webkitAudioContext();
...
ChannelStrip = function() {
	...
	var gain = Zinedine.ctx.createGainNode();
	gain.gain.value = 1;
	that.output = gain;
	
	var in_gain = Zinedine.ctx.createGainNode();
	in_gain.gain.value = 1;
	that.input = in_gain;
	
	var prefader = Zinedine.ctx.createGainNode();
...
MainMixer = function() {
	...
	that.addSend = function(name, from, to, gainValue) {
		...
		var send = Zinedine.ctx.createGainNode();
		from.connect(send);
		send.connect(to);
...
Gen.OneshotSample = function(node, time, buffer, rate) {
	rate = rate || 1;
...
Gen.OscillatorStackBurst = function(node, time, freq, types, detunes, amps, duration) {
	var postGain = Zinedine.ctx.createGainNode();
	postGain.gain.setValueAtTime(1, time);
	
	for(var i = 0; i < types.length; i++) {
		
		var oscGain = Zinedine.ctx.createGainNode();
...
Gen.WhiteNoiseBurst = function(node, time, duration) {
	...
	player.noteGrainOn(time, offset, duration);
	player.noteOff(time + duration);
...
FX.FeedbackDelay = function(node, time, feedback) {
	...
	var delay = Zinedine.ctx.createDelayNode(time * 2);
	delay.delayTime.value = time;
	var fbGain = Zinedine.ctx.createGainNode();
...
FX.WaveShaper = function(node, preGain, postGain, array) {
	...
	var preGainNode = Zinedine.ctx.createGainNode();
	preGainNode.gain.value = preGain;
	
	var postGainNode = Zinedine.ctx.createGainNode();
...
Instrument = function(ev, defaults) {
	...
	that.output = Zinedine.ctx.createGainNode();
...
Instr.LFOscillator = function(ev) {
	...
	player.noteOn(ev.startTime);
	player.noteOff(ev.startTime + ev.len);
...
// TODO: RessourceLoader should call start, if no ressource is necessary.

RessourceLoader = function(base) {
...
DefaultQuantClock = undefined;
QuantClock = function(tempo, latency) {
	tempo = tempo || 120;
	latency = latency || 0.2;
	var that = {};
	var beatsPerBar = 4;
	...
	that.startTime = Zinedine.ctx.currentTime;
	...
	that.update(that.startTime + latency);
	...
	that.audioTime = function(beats) {
		return that.startTime + (beats / bps) + pausedTime;
	};
	
	that.audioClientBeatTime = function(beats) {
		return that.startTime + (beats / bps) - latency + clientCompensation;
	};
